...
- **情境自訂**：用戶只需輸入情境描述（如「無奈地加班」、「在雨中漫步」），系統自動建構完整 prompt
- **API Key 管理**：支援用戶自備 Google AI Studio API Key，或使用預設 Key（附帶每日 10 次限額）
- **公開貼圖牆**：所有人產生的貼圖儲存於 server 端，供所有訪客瀏覽與下載，超過 10 張自動分頁；每張貼圖完整顯示情境描述（最多 30 字）與作者名稱
- **一鍵下載**：生成後或從貼圖牆皆可直接下載 PNG 圖片
- **原創貼圖連結**：頁面底部展示囧雞原創 LINE 貼圖封面，並附連結至 LINE Store
...
1. 開啟網頁後，在輸入框中描述你想要的情境（最多 30 字，例如：「無奈地加班」、「在雨中漫步」）
2. （選配）在作者欄位輸入你的名字，留空則顯示「匿名」
3. （選配）展開「API Key 設定」區塊，輸入你自己的 Google AI Studio API Key 以解除每日限制
4. 按下「產生貼圖」按鈕（注意：生成結果將會公開且標記作者）
5. 等待生成完成後，可預覽圖片並點擊下載按鈕儲存
6. 頁面底部「大家產生的貼圖」牆會自動更新，所有訪客都可瀏覽與下載，完整顯示最多 30 字的情境描述與作者名稱
